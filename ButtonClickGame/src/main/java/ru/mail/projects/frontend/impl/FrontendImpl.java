...
public class FrontendImpl extends AbstractHandler implements Runnable, Frontend {
	...
	public void replicateFromGM () {
		...
		for (UserSession usSes : sessions.values()) {
			if (usSes.game_state == true && usSes.timeToFinish > 0) {
                usSes.timeToFinish -= 100;
            }
        }
	}
	...
	public void handle(String target, Request baseRequest,
		...
		if (doAuthentification (baseRequest, request, response) == AuthorState.NotEnd) {
            return;
        }
	...
	public void startGame (Request baseRequest, HttpServletRequest request,
		...
		if (sessions.get(idSess.getLong()).game_state == true){
            ...
            PageGen.generateWaitPage(sessions.get(idSess.getLong()), response, baseRequest);
        }
	}
	...
	public AuthorState doAuthentification (Request baseRequest, HttpServletRequest request,
		...
		if (Login == null && Reg == null && RegLogin == null) {
            showStartPage (baseRequest, response, request);
        }
		//Выводим форму для регистрации
		else if (Reg != null) {
            showRegistrationForm (baseRequest, request, response);
        }
		//Делаем авторизацию
		else if (Login != null) {
            autorizeUser (baseRequest, request, response, Login);
        }
		//Делаем регистрацию
		else if (RegLogin != null) {
            registrateUser (baseRequest, request, response, RegLogin);
        }
	...
	public void showStartPage (Request baseRequest, HttpServletResponse response,
				...
				throws IOException, ServletException {
        UserSessResource usr = (UserSessResource)localRs.getResource("./resources/userSess.xml");
        UserSession UsSess = new UserSession(usr);
